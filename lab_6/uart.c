...
//    UART1_ICR_R &= 0xFFFFFFEF;
    // I think this is wrong and also unnecessary
    // seems to me that we should set bit 4 to a 1 in order to clear the interrupt (pg 928 & 934)
    // but also I think the interrupt is cleared by simply reading from UART1_DR_R in receive
}
